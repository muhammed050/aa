...
const body = z.object({
  username: username.optional(), sections: z.array(section).max(100).optional(), published: z.boolean().optional(),
  theme: z.record(z.string(), z.unknown()).optional(), name: z.string().min(1).max(120).optional(),
  bio: z.string().max(1000).nullable().optional(), template: z.number().int().min(1).max(8).optional(),
  seo_title: z.string().max(160).nullable().optional(), seo_description: z.string().max(320).nullable().optional(), noindex: z.boolean().optional(),
});
...
export async function PUT(req: Request) {
  ...
  const { sections, ...pageValues } = values;

  let { data: page, error: pageLookupError } = await supabase.from('pages').select('id').eq('user_id', user.id).maybeSingle();
  ...
  if (!page) {
    ...
    const { data: created, error } = await supabase.from('pages').insert({ user_id: user.id, username: u, name: values.name || 'صفحتي', published: values.published ?? false }).select('id').single();
    if (error) return NextResponse.json({ error: error.code === '23505' ? 'اسم المستخدم مستخدم بالفعل' : error.message }, { status: 400 });
    page = created;
  }

  if (Object.keys(pageValues).length) {
    const { error } = await supabase.from('pages').update({ ...pageValues, updated_at: new Date().toISOString() }).eq('id', page.id);
    if (error) return NextResponse.json({ error: error.code === '23505' ? 'اسم المستخدم مستخدم بالفعل' : error.message }, { status: 400 });
  }
